...
static int animate(event_t *event, int player)
{
    ...
    if (player < 0 || PLAYER_MAX <= player)
        return 0;
    event->tile->texture_rect.x = GAME.player[player].direction;
    event->tile->texture_rect.y = GAME.player[player].deltawalking;
    event->tile->texture_rect.y /= SEC_TO_MICRO / WALKING_ANIMATION_SPEED;
    event->tile->texture_rect.y %= 4;
    if (event->tile->texture_rect.y % 2 != 0)
        event->position.y -= (float)1 / TILE;
    if (event->tile->texture_rect.y == 2)
        event->tile->texture_rect.y = 0;
    if (event->tile->texture_rect.y == 3)
        event->tile->texture_rect.y = 2;
